...
const CustomerStatusDialog = ({ isOpen, customerId, customerName, currentStatus, onClose, onSuccess }: CustomerStatusDialogProps) => {
    ...
    const handleConfirm = async () => {
        try {
            setLoading(true)
            setApiError('')
            const res = await customerService.toggleStatus(customerId)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className={`relative bg-white rounded-lg shadow-xl w-full max-w-sm mx-4 p-6 transition-all duration-200 ${visible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                ...
                <p className="text-sm text-gray-600 mb-4">
                    Bạn có chắc muốn {isLocking ? 'khóa' : 'mở khóa'} tài khoản <strong>"{customerName}"</strong>?
                    {isLocking && (
